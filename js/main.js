...
const observerOptions = { threshold: 0.2, rootMargin: '-100px 0px -100px 0px' };
...
function initializeRadarChart() {
  ...
  function calculateRadarPoints() {
    ...
    dataPoints.forEach((point) => {
      const angle =
        (parseFloat(point.style.getPropertyValue('--angle')) * Math.PI) / 180;
      const value = parseFloat(point.style.getPropertyValue('--value'));
      points.push({ x: Math.cos(angle) * value, y: Math.sin(angle) * value });
    });
    return points;
  }
...
const observerOptions2 = { threshold: 0.5, rootMargin: '0px' };
